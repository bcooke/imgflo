...
- `openai`: Already proven useful, commonly needed, acceptable dependency size
...
- `imgflo-vega` - Vega-Lite visualizations (vega-lite dependency)
- `imgflo-plotly` - Plotly charts (plotly dependency)
- `imgflo-chartjs` - Chart.js with node-canvas (canvas dependency)
...
- **PlantUML**: UML diagrams (if someone builds it)
- **Graphviz**: Graph layouts (if someone builds it)

### AI Generation
- **OpenAI**: General images, photos, illustrations
- **Stability**: Alternative AI (if someone builds it)
- **Midjourney**: Via API (if someone builds it)
...
## Future: Official Generator Packages

When we create official generator packages:
